##########################
#
# Get the available subnets that are attached to our account
#
##########################
require 'json'
...
begin
  ...
  subnets = Azure::Armrest::Network::VirtualNetworkService.new(config)
  getsubs = subnets.get(vnet, rg)
  names   = getsubs.properties.subnets.map { |x| [x['id'], x['name']]}

  $evm.log(:info, "Resource groups:|#{names}")
